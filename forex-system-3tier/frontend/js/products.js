async function loadProducts(){
    const{data}=await db.from('products').select('*').order('product_id');
...
async function addProduct(){const name=document.getElementById('mName').value.trim(),hs=document.getElementById('mHs').value.trim(),price=parseFloat(document.getElementById('mPrice').value);if(!name||!price){showToast('Name and price required','error');return;}const{error}=await db.from('products').insert([{product_name:name,hs_code:hs,unit_price:price}]);if(error){showToast(error.message,'error');return;}showToast('Product added!');closeModal();loadProducts();}
async function deleteProduct(id){if(!confirm('Delete?'))return;await db.from('products').delete().eq('product_id',id);showToast('Deleted');loadProducts();}
loadProducts();
